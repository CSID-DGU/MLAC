...
X_test=pd.read_csv(os.path.join(cici_test_dir,'X_test3.csv'))
...
L2_ytest=X_test['nist_category3'].copy()

L3_ytest=X_test['attack_category'].copy()

c1_Xtest=X_test.query('nist_category3==1')
c1_ytest=c1_Xtest['attack_category']

c2_Xtest=X_test.query('nist_category3==2')
c2_ytest=c2_Xtest['attack_category']

c3_Xtest=X_test.query('nist_category3==3')
c3_ytest=c3_Xtest['attack_category']

c4_Xtest=X_test.query('nist_category3==4')
c4_ytest=c4_Xtest['attack_category']

for class_data in [c1_Xtest,c2_Xtest,c3_Xtest,c4_Xtest]:
    class_data.drop(labels=['Unnamed: 0','attack_category','nist_category','nist_category3'],axis=1,inplace=True)

X_test.drop(labels=['Unnamed: 0','nist_category','attack_category','nist_category3'],axis=1,inplace=True)
...
try:
    ...
    os.system("sudo chmod 777 /home/irteam/junghye-dcloud-dir/MLAC/230719/result/CICI/v3/progressLog.txt")
    f=open(progressLog_path,'w')
    f.truncate()
